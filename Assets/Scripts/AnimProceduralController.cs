...
public class AnimProceduralController : MonoBehaviour
{
    ...
    Vector3[] MatchToSurfaceFromAbove(Vector3 point, float halfRange, Vector3 up)
    {
        ...
        if (Physics.SphereCast(ray, sphereCastRadius, out hit, 2f * halfRange,targetLayer))
        {
            res[0] = hit.point;
            res[1] = hit.normal;
        }
    ...
    public void resetLegs() {
        ...
        for (int i = 0; i < nbLegs; ++i)
        {
            ...
            var temp = smoothness;
            smoothness = 1;
            legTargets[i].position = baseLegPos[i];
            smoothness = temp;
            legMoving[i] = false;
        }
        isResetting = false;
